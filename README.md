...
- `filler-sessions/` — standalone sessions sitting outside the 9-term plan, of two kinds: complete-in-one-sitting badges (each finishes a full, real Scout Activity Badge in a single sitting) and passive-progress badges (`outdoor-challenge-award/`, `adventure-challenge-award.md`, `teamwork-challenge-award.md`, `paddle-sports-staged-activity-badge.md` — badges the group decided to never dedicate a whole term to, earned instead across many terms via the standing fixed slots above plus these sessions). See `filler-sessions/README.md`.
...
`year-1/term-1/` (World Challenge Award), `year-1/term-2/` (Emergency Aid Staged Activity Badge, Stage 3), `year-1/term-3/` (Chef Activity Badge), `year-2/term-1/` (Creative Challenge Award), and `year-2/term-2/` (Pioneer Activity Badge) are the terms started so far — Year 1, Term 1 in full session-level detail, all the others at framework level only (theme and badges in `data/terms.yaml` plus a placeholder `overview.md`; session-by-session detail is still to come). Year 3 doesn't appear in `data/terms.yaml` at all yet — its old content was a pre-redesign leftover that had never been revisited, so it's been removed rather than kept as stale placeholder data (see the Status note below), and will be added back once it's actually replanned. Year 2, Term 3 also doesn't appear — see below.
...
Year 3's three terms have also been removed from `data/terms.yaml`, separately from the Year 4 drop above. Unlike Year 4, Year 3 isn't being dropped from the programme — its old content was simply a pre-redesign leftover from the original Google Doc import that had never actually been revisited, and had known conflicts (it duplicated Chef Activity Badge and Emergency Aid Stage 3/4 from Year 1, and still listed Teamwork Challenge Award as a dedicated term badge despite that award being made permanently passive). Rather than keep showing that as if it were a real plan, it's been removed until Year 3 is genuinely replanned, the same way Year 1 and Year 2 were. Badges that only had a home in the old Year 3 (Skills Challenge Award, Personal Challenge Award, Communicator Activity Badge, Master at Arms Activity Badge) are flagged "pending Year 3 replan" in `data/awards.yaml`.

Year 2, Term 3 ("On the Water") has also been removed from `data/terms.yaml`. Its badge, Paddle Sports Staged Activity Badge (Stage 1), moved to passive — a fourth badge alongside Outdoor/Adventure/Teamwork Challenge Award, earned via `filler-sessions/paddle-sports-staged-activity-badge.md` plus every summer term's standing Time on the Water/External Activity slot — because the group already runs paddle sports content every summer regardless of any dedicated badge term, so a whole term for it was redundant. That left Year 2, Term 3 without a focus badge, so it's been removed pending a new theme and badge, not yet decided.
...
1. **Deliberate design** — Outdoor Challenge Award, Adventure Challenge Award, Teamwork Challenge Award, and Paddle Sports Staged Activity Badge (Stage 1) are never given a dedicated term; they're earned entirely through the standing fixed slots and the passive-progress sessions in `filler-sessions/`.
