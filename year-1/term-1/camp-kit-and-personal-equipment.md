# Camp Kit & Personal Equipment

**Term:** Year 1, Term 1 — Campcraft & Outdoor Skills

**Fits the term because:** before Scouts can pitch a tent or cook on camp, they need to know what to bring and how to carry it — the practical starting point for the term's campcraft focus.

**Badge focus:** Outdoor Challenge Award optional extra — rucksack packing. It's one of the "complete at least four" tasks (alongside the menu planning at [Camp](camp.md) and the campfire entertainment in [Fire Safety & Camp Cooking Prep](fire-safety-and-camp-cooking.md)).

> "Show how to pack a rucksack correctly, with appropriate kit for the camp or event."

## Running order

| Time | Duration | Block |
| --- | --- | --- |
| 0:00–0:10 | 10 min | Welcome |
| 0:10–0:25 | 15 min | Kit list bingo |
| 0:25–0:55 | 30 min | Practice packing an expedition rucksack |
| 0:55–1:20 | 25 min | Everything you need, nothing you don't |
| 1:20–1:40 | 20 min | Kit inspection parade |
| 1:40–1:50 | 10 min | Patrol kit checklist |
| 1:50–2:00 | 10 min | Close |

## Session detail

### Welcome (10 min)

Notices, patrol flags, opening.

### Kit list bingo (15 min)

Rather than read out a kit list, turn it into bingo. Hand each Scout a card with nine kit items in a grid (sleeping bag, torch, waterproofs, spare socks, etc.). Read out clues instead of item names — "you'll wish you had this if you need the loo at 2am" (torch), "your feet will thank you for a spare pair" (socks) — and Scouts cross off as they guess. First to a line shouts "camp!" instead of "house." This gets the full kit list covered with the group actively guessing rather than being talked at.

**Materials:** bingo cards, clue sheet.

### Practice packing an expedition rucksack (30 min)

[Practice packing an expedition rucksack](https://www.scouts.org.uk/activities/practice-packing-an-expedition-rucksack/) — using the activity's kit list (roll mat, water bottle, clothes, tent, cooking kit, sleeping bag, stove, wash kit, waterproofs, first aid kit, and so on), patrols race to pack a shared rucksack correctly and efficiently: heavy items low and central, frequently-needed items accessible, nothing dangling off the outside. Run it as a timed relay between patrols, then have each patrol explain their packing order to the room — the reasoning is the actual learning, not just the speed.

**Materials:** rucksacks, borrowed/group camp kit to pack (one full kit's worth per patrol).

### Everything you need, nothing you don't (25 min)

[Everything you need, nothing you don't](https://www.scouts.org.uk/activities/everything-you-need-nothing-you-don-t/) — read out a scenario (a weekend camp, a day hike, a rainy paddleboarding session) and a list of possible items. For each item, patrols hold up a green or red card for "pack it" or "leave it," then justify a few of their answers. Points for correct calls, bonus points for the best justification of a tricky one (does a hairdryer count? no).

**Materials:** green/red cards, item list, scenario cards.

### Kit inspection parade (20 min)

Run this as a mock formal inspection, played for laughs — a leader takes on an exaggerated "sergeant-major" character, walking the line with a clipboard, checking each Scout has their named kit list items and enforcing (gently, comically) that a name goes on everything. Follow with a name-tape labelling race: patrols compete to correctly label a pile of mixed kit against the clock.

**Materials:** name tapes/labels, marker pens, clipboard.

### Patrol kit checklist (10 min)

Each patrol writes up their own kit checklist for the weekend camp on a single sheet, splitting who's bringing shared items (stove, first aid kit, tent poles) so nothing gets duplicated or missed. Hand these in — they get handed back for the final kit check on arrival at [Camp](camp.md).

### Close (10 min)

Notices, thought for the week, home.

## Notes

- Bring a selection of rucksacks and camp kit (borrowed group kit is fine) for the packing practice.
- The patrol kit checklists produced here feed into the final kit check before the optional weekend camp.
